...
class Cnarration
  class << self
    ...
    def pages_a_relire options = nil
      pars = pages(where: "options LIKE '16%'", as: :array_data, colonnes: [:titre, :handler, :options], sorted: true)
      if pars.empty?
    ...
    def pages_derniere_lecture options = nil
      pdls = pages(where: "options LIKE '18%'", as: :array_data, colonnes: [:titre, :handler, :options], sorted: true)
      if pdls.empty?
    ...
    def build_page_list hdata, next_level
      hdata.collect do |bid, bdata|
        ...
        bdata[:pages].collect do |hpage|
          # debug "--- hpage : #{hpage.inspect}"
          priorite = hpage[:options][3].to_i
          li_for_page( hpage, next_level, priorite )
        end.join('')
      end.join.in_ul(class: 'tdm pages books_discrets')
    end

    # Retourne le code HTML du LI pour une page à corriger
    def li_for_page hpage, next_level, priorite
      # Définit la class css en fonction de la priorité de
      ...
      (btns_page_edition(hpage, next_level)+line_titre).in_li(value: hpage[:id], class: css.join(' '))
    end
