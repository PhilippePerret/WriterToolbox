...
class User
  ...
  def check_abonnement
    if abonnement_recent?(6)
      "#{user.pseudo}, puisque vous venez récemment de vous inscrire — moins de 6 mois —, votre abonnement au site de #{site.tarif_humain} est déduit de votre inscription de #{Unan::tarif_humain} au programme #{Unan::PROGNAME_MAJ}. Vous n'avez donc que <strong>#{tarif_unanunscript.as_tarif}</strong> à payer ! :-)"
    else
  ...
  # UN AN (dans les deux minutes qui précèdent ou le temps
