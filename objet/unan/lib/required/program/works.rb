...
class Program
  ...
  def current_works options = nil
    ...
    case options[:as]
    when :data        then @arr_works
    when :id, :ids    then @arr_works.collect{|h| h[:id]}
    when :instance, :instances
      @arr_works.collect { |hwork| Work::get(self, hwork[:id]) }
    else
      h = {}
      @arr_works.each{ |dwork| h.merge!( dwork[:id] => dwork) }
      h
    end
  end
  ...
  def ulterieurs_works options = nil
    ...
    case options[:as]
    when :data      then @ulterieurs_works
    when :id, :ids  then @ulterieurs_works.collect{|h| h[:id]}
    when :instance, :instances
      @ulterieurs_works.collect{ |dwork| Work::get(self, dwork[:id]) }
    else
      h = {}
      @ulterieurs_works.each{ |dwork| h.merge!( dwork[:id] => dwork) }
      h
    end
  end
  ...
  def works_by_type type, options = nil
    ...
    where = ["CAST(SUBSTRING(options,1,2) as UNSIGNED) IN (#{codes_by_type.join(',')})"]
  ...
  def works filtre = nil
    @works ||= self.table_works.select(order:"created_at DESC AND program_id = #{id}")
  ...
  def last_tasks
    @last_tasks ||= begin
      ...
      where_clause = "status = 9 AND program_id = #{id}"
      # Je relève les 20 derniers travaux (work) de l'auteur qui
      # ont été terminés
      res = self.table_works.select(colonnes: [:abs_work_id], where:where_clause, order:"created_at DESC", limit:20)
