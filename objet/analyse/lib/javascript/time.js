...
window.Time = {
  ...
  */
  secondsToHorloge:function(val,options){
    ...
    if('undefined'==typeof options.hour_if_0) options.hour_if_0 = true ;
    var fms="000";
    ...
    var with_hours = hrs > 0 || options.hour_if_0 ;
    if((options.hour_2_digits || (this.FORMAT_HORLOGE_DEFAULT & HOUR_TWO_DIGITS)) && hrs < 10) hrs = "0"+hrs;
    var reste = val % 3600;
    var mns = Math.floor(reste / 60);
    if(with_hours && mns < 10) mns = "0"+mns;
    var scs = (reste % 60) * 1000;
    scs = Math.floor(scs/1000);
    if(scs<10) scs = "0"+scs
    var h = "" ;
    if ( with_hours ) h += hrs+(options.unit_hours|| ":") ;
    h += mns+(options.unit_minutes||":")+scs+(options.unit_seconds || "")
