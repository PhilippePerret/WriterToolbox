...
personnelle pour le programme UNAN.

Pour enregistrer une valeur dans cette table on utilise
l'extension Unan :

  User.set_unan_var <name>, <valeur>

<valeur> doit être un numbre fixnum, un string ou une valeur
booléenne.

=end
def schema_table_unan_variables(user_id)
  # debug "-> schema_table_unan_variables(#{user_id}) (création table 'variables_#{user_id}')"
  <<-MYSQL
