...
class CRON2
  class ConnexionsReport
    ...
    def initialize
        @report = Hash.new
        @report = {
            by_ip:      Hash.new, # à supprimer quand suivant ok
            by_ip2017:  Hash.new, # par instance
            message:    nil,
            titre:      nil
        }
    end
    ...
    def recolte_connexions_by_ip
        ...
        all_connexions.each do |con|
            ...
            # Formule 2017 par instances
            # ------------------------------
            iconnexion = Connexions::Connexion.new(con[:route], con[:time], con[:ip])
            @report[:by_ip2017][ip] ||= Connexions::IP.new(ip)
            @report[:by_ip2017][ip].add_connexion iconnexion
        end
    ...
    # La rationalisation des connexions consiste à voir
    # si l'on connait le propriétaire de l'IP, lorsque c'est
    # un moteur de recherche ou autre adresse IP connue.
