...
begin
  ...
  Dir.chdir(OFFLINE ? "/Users/philippeperret/Sites/WriterToolbox": '.') do
    ...
    dbm_table = SiteHtml::DBM_TABLE.new(:forum, 'sujets')

    # RÉCUPÉRATION DES DONNÉES ORIGINALES
    # -----------------------------------
    original_data = {}
    pr = dblite.prepare('SELECT * FROM sujets')
    pr.execute.each_hash do |row|
      original_data.merge! row['id'] => row.to_sym
    end

    pr2 = dblite.prepare('SELECT * FROM sujets_posts')
    pr2.execute.each_hash do |row|
      next unless original_data.key?(row['id'])
      original_data[row['id']].merge!(row.to_sym)
    end
    ...
    original_data.each do |rid, hdata|
      # ---------------------------------------------------------------------
