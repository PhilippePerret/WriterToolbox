...
begin
  ...
  Dir.chdir(OFFLINE ? "/Users/philippeperret/Sites/WriterToolbox": '.') do
    ...
    Dir["./database/data/unan/user/*"].each do |ufolder|
      ...
      next if dbpath.nil?
      program_id = File.basename(dbpath, File.extname(dbpath)).sub(/^programme/,'').to_i
      debug "PROGRAM ID : #{program_id}"
      debug "   - dbpath: #{dbpath}"

      dblite = SQLite3::Database.new(dbpath)
      # La table dans la base MySQL
      table_name = "unan_quiz_#{uid}"
      ...
      # -----------------------------------
      begin
        pr = dblite.prepare('SELECT * FROM quiz')
      rescue Exception => e
        # Pas de table variables pour ce user
        debug "# Pas de table => je passe au suivant"
        next
      end
      original_data = []
      ...
      if original_data.empty?
        debug "= Pas de données => passage au suivant"
        next
      end
      ...
      original_data.each do |hdata|
        ...
        # ---------------------------------------------------------------------
        dbm_table.insert( hdata.merge( program_id: program_id ) )
      end
