...
begin
  ...
  Dir.chdir(OFFLINE ? "/Users/philippeperret/Sites/WriterToolbox": '.') do
    ...
    # La base SQLite
    dbpath = './database/data/site_cold.db'
    dblite = SQLite3::Database.new(dbpath)
    # La table dans la base MySQL
    dbm_table = SiteHtml::DBM_TABLE.new(:cold, 'permanent_tweets')

    # RÉCUPÉRATION DES DONNÉES ORIGINALES
    # -----------------------------------
    pr = dblite.prepare('SELECT * FROM permanent_tweets')
    original_data = []
    ...
    original_data.each do |hdata|
      ...
      # if hdata[:created_at].nil?
      #   hdata[:created_at] = Time.now.to_i - 5 * 3600 * 24
      # end
      # if hdata[:updated_at].nil?
      #   hdata[:updated_at] = Time.now.to_i - 5 * 3600 * 24
      # end
      # ---------------------------------------------------------------------
