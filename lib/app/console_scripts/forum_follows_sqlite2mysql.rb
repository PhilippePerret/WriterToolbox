...
begin
  ...
  Dir.chdir(OFFLINE ? "/Users/philippeperret/Sites/WriterToolbox": '.') do
    ...
    dbm_table = SiteHtml::DBM_TABLE.new(:forum, 'follows')

    # RÉCUPÉRATION DES DONNÉES ORIGINALES
    # -----------------------------------
    pr = dblite.prepare('SELECT * FROM sujets_followers')
    original_data = []
    ...
    original_data.each do |hdata|
      ...
      hdata.delete(:id)
      hdata[:items] = JSON.parse(hdata.delete(:items_ids)).join(' ')

      if hdata[:user_id] != nil && hdata[:sujet_id] != nil
        hdata[:time]  = hdata[:items].to_i
        hdata[:items] = nil
      end
      debug "--- #{hdata.inspect}"
      # ---------------------------------------------------------------------
