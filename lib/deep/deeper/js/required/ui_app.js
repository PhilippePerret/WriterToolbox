...
$.extend(window.UI,{
  ...
  /** Méthode permettant de bloquer la page, i.e. de rendre
    ...
    *
    * options
    *   minimize_textarea   Si true (défaut), les textarea sont minimisés et
    *                       s'aggrandissent quand on focus dedans.
    */
  bloquer_la_page:function(bloquer, options){
    if(undefined==options){options={}}
    if(undefined==options.minimize_textarea){options.minimize_textarea=true}
    var css_content = {}
    if(bloquer){
    ...
    if(bloquer){
      ...
      /**
        * On réduit les textarea et on place des observers sauf si l'option
        * 'minimize_textarea' est false
        */
      if (options.minimize_textarea){
        $('textarea').css({
